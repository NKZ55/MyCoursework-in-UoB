...
I could not contact my teammates in time as I was studying remotely, so this report was all done independently.
...
Plus, we use the `multiplot` function, courtesy of [R Cookbooks](http://www.cookbook-r.com/Graphs/Multiple_graphs_on_one_page_(ggplot2)/) to create multi-panel plots.
...
Although some variables have outliers, such as `dep_delay` and `arr_delay`, we will not remove these outliers because it’s essential to use extreme values to analyse the causation of departure delay.
...
It can be seen that there is a correlation between delays and the total number of flights. In other words, the occurrence of departure delays may depend on how busy the airport is. We will further analyse the influence of busyness across time.

# Influence of busyness
...
There is __no clear link__ between the total number of flights and the percentage of delayed flights across a day. We will be able to analyse the effect of busyness more accurately when we have access to data on the number of flights arriving at these airports.

However, we notice that the proportion of delayed flights tends to __increase gradually over the day__. This may be due to the accumulation of flight problems that are not resolved promptly throughout the day.

# Seasonal infuences

The trend in flight departure delays over the year.
...
We note that the percentage of delayed flights is high in July and December and decreases between September and November, indicating that seasonal influences do exist.

We got the `weather` data to analyse seasonal influences.
...
Among the weather factors, the precipitation and visibility are two possible factors that can significantly affect the departure delay.

## Influence of precipitation and visibility
...
We can observe a correlation between weather condition and the percentage of delayed flights. The less rainfall or the better the visibility, the lower the percentage of flight delays will be. And only prolonged periods of good weather can significantly impact the percentage of flight delays, which can explain the drop in the percentage of flight delays between September and November. With the ever-changing weather, brief periods of good weather have very little effect on the percentage of flight delays. (The *Delayed Percent vs Weather* sheet in Tableau presents this relationship more clearly)
...
According to the table and graph, US perform the best and most stable in departure delay, while MQ, DL and AA should control the maximum length of delays, and WN, VX, EV, B6 and 9E should focus on reducing the percentage of departure delays.
...
Older planes may require a longer checking time before departure.
...
The percentage of delayed flights does not have a strong relationship with the year manufactured of the plane. Meanwhile, we notice that some older planes with a similar number of flights to new planes have a smaller percentage of delays, which is counter-intuitive.

Combined with Tableau, we find that even for planes built during the same period, carriers have considerable differences in performance (See *Carrier's Plane Year* sheet in Tableau). The poorer performing planes in the chart above are used heavily by the carriers as the poor performers. As we cannot think of any other reason for the new produced planes to have a higher percentage of flight delays, we suspect that the carriers are likely to be the real reason behind this. Of course, with the aid of professionals from the client company, we will be able to draw more accurate conclusions.

## Influence of seat number (i.e. plane size)

Bigger planes may require a longer checking time and passengers may take longer to board before departure.
...
Relationship between the percentage of delayed flights and the seat number is not clear. We use Tableau to precisely analyse each seat count and still conclude that the carrier is the actual cause (See *Carrier's Plane Seat* sheet in Tableau).

# Suggestions to PANYNJ

__Effect of busyness__. Although the percentage of departure delays at each of the three airports respectively correspond to their total number of flights, in subsequent in-depth studies, we did not find a strong correlation between the percentage of departure delays and the busyness of the airports. We need __information on arrivals at the three airports__ to explore this matter more accurately. However, we have seen an increasing trend in the percentage of flight delays throughout the day (*Figure 1*), we therefore recommend that when airports encounter problems with departure delays, in addition to __taking action as soon as possible__, they should also consider trying __not to disrupt flights in the next slot__. In the Tableau Dashboard, we can explore how each carrier deals with flight delays across the day.

__Effect of weather__. The proportion of flight delays declined during long periods of low rainfall and high visibility. To their credit, the airports and carriers __have done a great job with the weather impact on flights__, with most flights being scheduled in times of good visibility and no rainfall (*Figure 2*). 

__Effect of carrier__. US performs the best and most stable departure delay (*Figure 3*). Combined with the analysis of the dashboard in Tableau, we find that the percentage of delays varies from one destination airport to another. As UA and WN do not control their percentage of delays well, some destination airports with a higher volume of flights have a higher percentage of delays than others. US and AA airlines, on the other hand, have reduced the percentage of delays at some destination airports. Overall, we recommend PANYNJ to work more intensively with __US and AA Airways__, and conduct a reasonable distribution of flights from good and bad carriers for different destinations.